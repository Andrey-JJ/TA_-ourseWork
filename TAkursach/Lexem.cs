...
namespace TAkursach
{
    public class Lexem
    {
        ...
        List<(char, int)> tableTokens = new List<(char, int)>();
        //Свойство получающее значение для переменной tableTokens
        public List<(char, int)> TableTokens { get => tableTokens; private set => tableTokens = value; }
        #endregion
        ...
        int GetTokensForlexem(int type, int i) {
            switch (type) {
                ...
                case 3:
                    ...
                    tableTokens.Add(('V', codeVariables.IndexOf(LexemMap[i].Item1)));
                    type = 0;
                    break;
                case 4:
                    if (!codeLiterals.Contains(LexemMap[i].Item1))
                        codeLiterals.Add(LexemMap[i].Item1);
                    tableTokens.Add(('L', codeLiterals.IndexOf(LexemMap[i].Item1)));
                    type = 0;
                    break;
            }
            return type;
